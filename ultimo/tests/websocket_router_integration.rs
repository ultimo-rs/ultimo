...
async fn test_websocket_echo() {
    ...
    // Send message
    ws.send(TungsteniteMessage::Text("hello".to_string())).await.unwrap();
...
async fn test_websocket_binary_messages() {
    ...
    ws.send(TungsteniteMessage::Binary(data.clone())).await.unwrap();
    ...
    ws.send(TungsteniteMessage::Text("ping".to_string())).await.unwrap();
