...
Block or allow requests by client IP using CIDR notation. Respects
`trust_proxy` when enabled.

```rust
use ultimo::middleware::builtin::IpFilter;

// Allow only private networks + localhost:
...
Denied requests receive a `403 Forbidden` response. Supports IPv4 and IPv6,
bare IPs (treated as `/32` or `/128`), and any valid prefix length.
...
```rust
// Cargo.toml: ultimo = { version = "0.9", features = ["csrf"] }
