...
```toml
[dependencies]
ultimo = { version = "0.9", features = ["session"] }
```
...
Other methods: `remove(key)`, `id()`. `clear()` wipes all session data and —
like `destroy()` — removes the store entry and expires the cookie, so it
doubles as a logout call.
...
- **Anti-DoS** — empty/untouched sessions are never persisted and get no cookie.
  `MemoryStore` is unbounded by default; use `MemoryStore::with_max_sessions(n)`
  in production to cap live sessions (soonest-to-expire evicted first) and
  bound worst-case memory under sustained load.
...
`MemoryStore` (single-process) is built in — `MemoryStore::new()` is unbounded;
`MemoryStore::with_max_sessions(n)` caps it, evicting the soonest-to-expire
session to make room once full. Implement `SessionStore` for other backends:
