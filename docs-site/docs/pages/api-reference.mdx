...
##### `mount_rpc(&mut self, path: &str, rpc: RpcRegistry)`

Mount an `RpcRegistry` at `path` as a single JSON-RPC 2.0 `POST` endpoint —
single calls, batches, and notifications, per spec. JSON-RPC mode
(`RpcMode::JsonRpc`, the default) only; REST mode needs one route per
procedure instead.
...
- `client-gen` - Derive RPC client TypeScript types from Rust types via `ts-rs`; `query`/`mutation` infer types from `#[derive(TS)]` structs (string-typed `query_with_types`/`mutation_with_types` remain as escape hatches) ([RPC](/rpc), [TypeScript Client Generation](/typescript))
