...
restrict the handshake's `Origin` header with `WebSocketConfig::allowed_origins`:
...
`allowed_origins` is empty by default (no restriction — matches prior
behavior). Set it whenever the connection carries ambient authority. `"*"`
matches any origin; otherwise the match is exact against the full `Origin`
value. Once set, a handshake missing the `Origin` header is rejected with
`403 Forbidden`.
