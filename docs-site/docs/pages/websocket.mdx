...
Connect directly to the backend origin like this in development, rather than
through a frontend dev server's proxy (e.g. Vite's `server.proxy`) — those
proxies are built for short-lived request/response traffic and don't handle
long-lived WebSocket/SSE connections well. See
[SSE → Vite dev proxy buffers streaming responses](/sse#vite-dev-proxy-buffers-streaming-responses)
for the full explanation (same underlying issue).
...
Browsers don't enforce same-origin for WebSocket connections the way they do
for `fetch`/XHR — a page on any origin can open a WebSocket connection to
your server. If the connection relies on ambient cookie authentication,
restrict the handshake's `Origin` header with an allow-list:
...
The allow-list is empty by default (no restriction — matches
`websocket_with_config`'s prior behavior). Set it whenever the connection
carries ambient authority. `"*"` matches any origin; otherwise the match is
exact against the full `Origin` value. Once set, a handshake missing the
`Origin` header is rejected with
`403 Forbidden`.
