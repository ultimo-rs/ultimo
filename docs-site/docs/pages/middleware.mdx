...
app.use_middleware(|ctx, next| async move {
    ...
    if !auth_header.starts_with("Bearer ") {
        return Err(ultimo::Error::Unauthorized(
...
app.use_middleware(|ctx, next| async move {
    if ctx.req.method() == "POST" || ctx.req.method() == "PUT" {
        ...
        if content_type != "application/json" {
            return Err(ultimo::Error::BadRequest(
...
app.use_middleware(|ctx, next| async move {
    match next().await {
        ...
        Err(e) => {
            ...
            if let ultimo::Error::DatabaseError(_) = e {
                return Err(ultimo::Error::InternalServerError(
