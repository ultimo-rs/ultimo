...
`logger()` only emits `tracing` events — it doesn't print anything itself. Without
a subscriber installed, those events go nowhere and you'll see no output at all,
silently. Install one before `app.listen(...)`, e.g. with the `tracing-subscriber`
crate:
...
(`ultimo new` scaffolds that use `logger()` already do this for you.)
...
```toml
ultimo = { version = "0.9", features = ["compression"] }
```
