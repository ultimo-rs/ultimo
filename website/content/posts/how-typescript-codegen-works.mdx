...
One of Ultimo's most powerful [features](/features) is automatic [TypeScript](https://www.typescriptlang.org) client generation. You define your API types in [Rust](https://www.rust-lang.org), register your RPC methods, run a single CLI command, and get a fully typed TypeScript client — no manual schema writing, no drift between server and client, no runtime validation overhead.
...
The foundation of Ultimo's codegen is [ts-rs](https://github.com/Aleph-Alpha/ts-rs) v12, a [procedural macro](https://doc.rust-lang.org/book/ch19-06-macros.html) crate that derives TypeScript type definitions from Rust types at compile time.
...
Ultimo's `RpcRegistry` is the bridge between your handler functions and the code generator. When you [register an RPC method](/blog/build-your-first-api-with-ultimo), the registry captures everything needed to generate the client:
...
The typical [dev workflow](/blog/build-your-first-api-with-ultimo) looks like:
...
Change a Rust struct → client regenerates in ~200ms (incremental) → TypeScript compiler catches any frontend breakage → fix it → [full-stack type safety](https://docs.ultimo.dev) maintained continuously.
...
1. **Custom proc macro** — Full control but enormous maintenance burden. Handling every [serde](https://serde.rs) attribute, generic bound, and edge case means reimplementing what ts-rs already does.
...
- **[JSON-RPC](https://www.json.org) doesn't map cleanly to OpenAPI.** OpenAPI models resources with HTTP methods; JSON-RPC models procedures. Forcing RPC into REST semantics loses information.
...
That said, Ultimo does support [OpenAPI for its REST routes](/features) — it's just not the path for RPC client generation.
...
[Ultimo](https://crates.io/crates/ultimo)'s approach keeps Rust structs as the single source of truth. There's no separate schema to maintain, no binary wire format to debug, and the generated client uses plain JSON over HTTP — debuggable with `curl` and browser DevTools.
...
The current pipeline is [production-ready](/blog/introducing-ultimo-v0-5) for TypeScript, but we're planning extensions:
...
[Ultimo's](https://github.com/ultimo-rs/ultimo) TypeScript codegen pipeline is a four-stage process built on a simple insight: if the Rust compiler already knows your types, there's no reason to describe them again in another language.
