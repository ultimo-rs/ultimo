...
Run `cargo build` to fetch dependencies. This will take a moment on the first run as it compiles Ultimo and its dependencies.
...
The `Ultimo` builder chains everything together: shared state, RPC registry, REST routes, and middleware. The JSON-RPC endpoint is automatically mounted at `/rpc` when you attach an `RpcRegistry`.
...
No manual API wiring, no fetch boilerplate, no type mismatches between frontend and backend. Change a field in your Rust struct, re-run `ultimo generate`, and the TypeScript compiler immediately flags any frontend code that uses the old shape.
...
- **Add a database** — Ultimo supports SQLx (Postgres, MySQL, SQLite) and Diesel out of the box. Enable the `sqlx-sqlite` feature and replace the in-memory `Vec` with a real database. See the [database guide](https://docs.ultimo.dev/database).

- **Add authentication** — Use JWT tokens or API keys with Ultimo's auth guards. The `jwt-auth` example in the repository shows the full pattern including token issuance, validation middleware, and protected routes.
...
- **Deploy** — Ultimo compiles to a single static binary. Build with `ultimo build --profile release`, copy the binary to your server, and run it. No runtime dependencies needed.

Check out the [full documentation](https://docs.ultimo.dev) and the [examples directory](https://github.com/ultimo-rs/ultimo/tree/main/examples) for more patterns and real-world usage.
