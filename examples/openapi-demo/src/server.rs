...
async fn main() -> ultimo::Result<()> {
    println!("🚀 Ultimo OpenAPI Server with Live Backend");
    println!();
    ...
    // Add CORS middleware (allows all origins)
    ...
    openapi.write_to_file("openapi-live.json")
        .expect("Failed to write OpenAPI spec");
    println!("✅ OpenAPI spec saved to: openapi-live.json");
    println!();
    ...
    // Serve Swagger UI at /docs (like FastAPI!)
    ...
    app.options("/api/getUser", |ctx: Context| async move {
        ctx.text("").await
    });

    app.options("/api/listUsers", |ctx: Context| async move {
        ctx.text("").await
    });

    app.options("/api/createUser", |ctx: Context| async move {
        ctx.text("").await
    });
